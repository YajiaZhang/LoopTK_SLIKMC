...
{
  CHECKRESIZE(a.m,a.n);
  gen_array2d_scale(MYGENARGS,GENARGS(a),c,
    m,n);
}
...
{
  CHECKEMPTY();
  gen_array2d_scale(MYGENARGS,c,m,n);
}
...
{
  CHECKEMPTY();
  gen_array2d_scale(MYGENARGS,Inv(c),m,n);
}
...
template <class T>
bool MatrixTemplate<T>::Read(File&f)
{
  int _m,_n;
...
template <class T>
bool MatrixTemplate<T>::Write(File&f) const
{
