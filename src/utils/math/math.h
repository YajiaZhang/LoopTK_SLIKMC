...
/** @ingroup Math
 * @brief Contains all definitions in the math directory.
 */
namespace Math {
...
inline double PseudoInv(double x, double eps=dZero) { return (FuzzyZero(x,eps)?Zero:One/x); }
...
inline float PseudoInv(float x, float eps=fZero) { return (FuzzyZero(x,eps)?Zero:One/x); }
